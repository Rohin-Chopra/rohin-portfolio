...
description: Learn about serverless image optimization on AWS for a faster and smoother user experince.
...
Before the Lambda, let's create the lambda layer it will use. Serverless.tf is an excellent package for this scenario; it makes it easy to provision layers and lambdas via Terraform, similar to the [serverless framework](https://www.serverless.com/).
...
In the above code, we have created a module via the `terraform-aws-modules/lambda/aws` module. To create the layer, you need to specify create_layer = true to be true. Although it is optional for clarity, we have mentioned the runtimes that the lambda layer is compatible with.
...
- Memory size has been increased to 2000 MB from the default 128 MB. Although Sharp is a highly-performing library, Lambda's computational power can still significantly affect its performance. Hence, to improve their performance, we have increased the memory; also, increasing memory for a lambda proportionally increases the amount of CPU, increasing the overall computational power available.
...
```tf
npm i -D @types/aws-lambda @types/node esbuild ts-node typescript
```
...
- ts-node: Enables you to run Typescript code without compiling it; we will use it to execute the build script, which we will write later.
...
- Entry points: Specifies the file that will be evaluated first, which will load subsequent files and packages.
...
- Outfile: This is the folder from which our Lambda's code will be packaged and deployed.
...
```
"build" : "tsc && ts-node --esm esbuild.config.ts"
```

To explore what the actual code we will be packaging to Lambda, you can run the following command and find it in the esbuild/index.js
